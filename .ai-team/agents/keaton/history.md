...
📌 Keaton (2026-02-08): Fixed vitest.config.js on PR #35 (Playwright E2E branch) — added `exclude: ['e2e/**', 'node_modules/**']` so `npm test` doesn't pick up Playwright spec files. Important: when adding e2e tests (Playwright, Cypress, etc.) to a Vitest project, always exclude their directories from vitest's test discovery to prevent conflicts between test runners. All 47 unit tests pass cleanly, build succeeds.
