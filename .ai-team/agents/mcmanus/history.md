...
- **2026-02-08:** Responsive polish (#19) — audited all 9 component/page files at 375px/768px/1024px breakpoints. Key fixes: (1) Typography scaling — hero `text-5xl` → `text-3xl sm:text-5xl md:text-6xl`, all page headings `text-3xl` → `text-2xl sm:text-3xl`, section headings `text-2xl` → `text-xl sm:text-2xl`. (2) Touch targets — hamburger button, footer social icons, SocialLinks, contact form submit button, back navigation links all got `min-h-[44px] min-w-[44px]`. (3) Hero spacing reduced on mobile (`py-12 sm:py-24`, `space-y-12 sm:space-y-20`). (4) Episode detail metadata row uses `flex-wrap` for small screens. (5) Contact inputs got `min-h-[44px]`. Items already correct: Layout container padding (`px-4 sm:px-6 lg:px-8`), episode/guest grid reflow, YouTube `aspect-video`, contact form `w-full` inputs, GuestDetail flex-col → flex-row. All 47 tests pass, build succeeds. PR #28.
